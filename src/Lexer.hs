...
readNumber [] lex = finishNumber [] lex
readNumber (c : rest) lex
  | c == '.' =
    if '.' `elem` lex then error $ lex ++ ".: malformed number"
    else readNumber rest $ lex ++ ['.']
  | isDigit c = readNumber rest $ lex ++ [c]
  | otherwise = finishNumber (c : rest) lex

finishNumber :: String -> String -> (Token, String)
finishNumber rest lex
  | last lex == '.' = error $ lex ++ ": malformed number"
  | otherwise = (NumberToken lex, rest)
